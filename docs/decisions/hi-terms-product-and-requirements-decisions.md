...
- Hi-Terms 是一个产品的[两个递进大阶段](../SSOT/glossary.md#两大阶段two-phase-model)：
  - **第一大阶段**：先把[终端能力](../SSOT/glossary.md#终端能力terminal-capabilities)和用户体验做好，逐步追平并超越 macOS Terminal / iTerm
  - **第二大阶段**：在终端产品基础上，把命令行工具[会话](../SSOT/glossary.md#会话session)做成可观察、可控制、可协作的[一等对象](../SSOT/glossary.md#一等对象first-class-object)
...
Hi-Terms 在产品定位上，首先是一个面向 macOS 的终端产品。第一大阶段的核心目标是在终端能力和用户体验上持续迭代，逐步做到与 macOS Terminal / iTerm 持平，并在部分场景超越。
...
Claude Code、Codex CLI 等 AI CLI 是 Hi-Terms 最重要、最能体现差异化价值的优先场景。

AI CLI 的优化跨越两个大阶段：

- **第一大阶段**：确保 AI CLI 在 Hi-Terms 中稳定运行，提供良好的基础体验
- **第二大阶段**：将 AI CLI 会话提升为一等对象，提供[会话级开放接口](../SSOT/glossary.md#会话级开放接口session-level-open-apis)，支持外部应用驱动
...
- 多轮交互顺畅
- 用户澄清回合的承接
- 结果回传、继续执行和停止控制（第二大阶段）

### 明确边界

> 详见[术语表 — 产品边界 Out of Scope](../SSOT/glossary.md#out-of-scope不做)。

- 不做 Prompt 优化器
- 不做提示词改写器
- 不做任务意图增强器
- 不替用户决定如何向 AI CLI 下指令
- 不把自然语言能力归到 Hi-Terms 本身
...
Hi-Terms 第二大阶段需要围绕会话级能力进行设计，至少应支持：
...
- 查询状态
- 中断、恢复、继续和停止
- 向第三方应用暴露关键事件，例如会话已启动、正在执行、等待用户输入、已完成、已失败

> 接口的具体定义参见[术语表 — Interaction Layer](../SSOT/glossary.md#interaction-layer) 和 [Tool Bridge](../SSOT/glossary.md#tool-bridge)。
...
愿景文档（`hi-terms-vision.md`）和需求文档（`hi-terms-requirements.md`）必须与以下表述保持一致：

### 第一大阶段约束

- Hi-Terms 首先是一个面向 macOS 的终端产品
- 第一大阶段的核心目标是终端能力和用户体验
- AI CLI 在第一大阶段的目标是稳定运行和基础体验优化
- 架构设计应为第二大阶段预留扩展能力

### 第二大阶段约束

- 命令行工具会话是一等对象
- 第三方应用驱动 AI CLI 会话是主场景，而不是附带示例
- 开放接口的对象是"会话"，不是匿名终端文本缓冲区
- AI CLI 优化仅限体验层，不包括 Prompt 优化

## 6. 当前默认结论

基于本次决策，当前默认结论如下：

- Hi-Terms 是一个面向 macOS 的终端产品，发展分为两个递进大阶段
- 第一大阶段聚焦终端能力和用户体验，逐步追平并超越 macOS Terminal / iTerm
- 第二大阶段在终端产品基础上，把 AI CLI 会话做成可被外部应用稳定驱动和协作的一等对象
- Hi-Terms 不承担 Prompt 优化类职责
