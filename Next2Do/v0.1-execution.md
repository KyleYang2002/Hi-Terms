...
gantt
    ...
    section Phase F
        F1-F6 集成验收           :active, f, 2026-04-07, 2d
```
...
| F1 | B01 构建验证 | F | pending | E4 ✅ | B01 | 需 macOS 环境 | — |
| F2 | B02-B07 终端功能验证 | F | pending | F1 | B02-B07 | 需 macOS 环境 | — |
| F3 | B08 稳定性验证 | F | pending | F2 | B08 | 需 macOS 环境 | — |
| F4 | B09 vttest 验证 | F | pending | F2 | B09 | 需 macOS 环境 | — |
| F5 | B10-B12 Session 验证 | F | pending | D4 ✅ | B10-B12 | 需 macOS 环境 | — |
| F6 | 性能验证 | F | pending | F2 | — | 需 macOS 环境 | — |
...
> 黄色节点为当前可立即开始的任务（依赖已满足）。

### 3.4 验收项覆盖矩阵

| 验收项 | 描述 | 关键任务 | 当前状态 |
|--------|------|---------|---------|
| B01 | 构建成功（Debug + Release，0 项目 warning） | F1 | 未验证 |
| B02 | Shell 启动（提示符、光标、可输入） | E2, E3, E4 | 未验证 |
| B03 | 基础命令（echo, ls, cd, cat） | E1, E2, E4 | 未验证 |
| B04 | TUI 应用（top, vim，鼠标交互，退出恢复） | E1, E2 | 未验证 |
| B05 | Ctrl+C 中断（sleep, cat 中断） | E1 | 未验证 |
| B06 | 滚动（seq 1 200，上下滚动） | E2 | 未验证 |
| B07 | ANSI 8 色（前景/背景 + bold/italic/underline） | B2 ✅ + E2 | 部分就绪（渲染逻辑完成，待 UI 验证） |
| B08 | 稳定性（50 命令无崩溃，<200MB RSS，0 leaks） | F3 | 未验证 |
| B09 | vttest 菜单 1-3 通过率 ≥ 80% | F4 | 未验证 |
| B10 | Session ID（唯一 UUID） | D1, D4, F5 | 未验证 |
| B11 | PTY 所有权（Session 持有 PTY，生命周期绑定） | D2, D4, F5 | 未验证 |
| B12 | Registry 查询（allSessions, session(for:), 线程安全） | D3, D4, F5 | 未验证 |
...
- [x] DefaultTerminalPipeline 编译通过（代码审查确认，待 macOS 编译验证）
- [x] PTY → Parser → Snapshot 集成测试编写完成（3 个测试：echo/lifecycle/dirtyRegion）
- [ ] `make test` 全部通过（需 macOS 环境）
- [ ] Git commit 记录
...
- [ ] 应用启动后显示终端窗口（需 macOS 环境验证）
- [ ] Shell 提示符可见（需 macOS 环境验证）
- [ ] 可输入字符并看到回显（需 macOS 环境验证）
- [ ] `make build` Debug + Release 均成功（需 macOS 环境）
- [ ] Git commit 记录
...
- [ ] B01-B12 全部通过
- [ ] 性能指标达标（≥30fps, ≥50MB/s 解析, <50MB RSS 增长, 0 leaks）
- [ ] `make ci` 全流程通过
- [ ] Git commit 记录
- [ ] Git tag: v0.1
