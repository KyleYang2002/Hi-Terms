...
| TerminalPipeline 协议 | 替换 stub 为完整实现 |
...
PTYProcess 的 DispatchIO 读取在收到 EOF 时触发退出流程：

```swift
// PTYProcess 内部
dispatchIO.read(...) { done, data, error in
    if done {
        // EOF：Shell 已退出
        var status: Int32 = 0
...
## 11. TerminalPipeline 协议扩展

### 11.1 协议变更

V0.1 需要扩展 TerminalPipeline 协议以支持渲染集成：

```swift
public protocol TerminalPipeline: AnyObject {
    var parser: any TerminalParser { get }
    var screenBuffer: ScreenBuffer { get }
    var dirtyRegion: DirtyRegion { get }          // V0.1 新增
    var renderCoordinator: RenderCoordinator { get } // V0.1 新增
...
> 注意：`TerminalPipelineStub` 同步更新以满足协议要求，但内部实现为 no-op，仅用于测试。
...
| 文件 | 修改内容 |
|------|---------|
| `TerminalCore/SessionTypes.swift` | SessionState.exited 增加退出码参数 |
| `TerminalUI/TerminalPipeline.swift` | 协议增加 dirtyRegion、renderCoordinator 属性 |
