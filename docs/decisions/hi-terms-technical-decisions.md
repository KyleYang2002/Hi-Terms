...
- 需要在 CI 中集成公证流程（`notarytool`）。CI 集成公证属于 v0.1 范围；v0.0 仅验证本地签名和 DMG 打包链路，参见 [V0.0 技术设计 §8](../design/hi-terms-v0.0-technical-design.md#8-构建与分发链路)
...
- **要求:** 作为 CI 的一部分，每次提交自动运行（本地自动化方案在 v0.0 建立，CI 集成在 v0.1，参见 [V0.0 技术设计 §6](../design/hi-terms-v0.0-technical-design.md#6-测试基础设施)）
...
- v0.0 建立基础测试骨架（XCTest 框架 + vttest 本地自动化方案），v0.1 搭建 CI 流水线并将测试集成到 CI。详见 [V0.0 技术设计 §6](../design/hi-terms-v0.0-technical-design.md#6-测试基础设施)
